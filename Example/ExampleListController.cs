...
public class ExampleListController : MonoBehaviour
{
    ...
    public enum DataMode { Observable, PlainList }
    ...
    IEnumerator LoadMoreRoutine()
    {
        ...
        if (toAdd <= 0) { isLoadingMore = false; yield break; }
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Toggle(Mode == DataMode.Observable, "Observable", "Button")) { if (Mode != DataMode.Observable) SetMode(DataMode.Observable); }
        if (GUILayout.Toggle(Mode == DataMode.PlainList, "PlainList", "Button")) { if (Mode != DataMode.PlainList) SetMode(DataMode.PlainList); }
        ...
        {
            Adapter.JumpToStart(ParseDurationOrDefault(_jumpDurationStr, JumpDurationDefault), JumpEase, ViewportPosToJumpPosition(_viewportPosIndex));
        }
        GUILayout.EndHorizontal();
        ...
        {
            Adapter.JumpToMiddle(ParseDurationOrDefault(_jumpDurationStr, JumpDurationDefault), JumpEase, ViewportPosToJumpPosition(_viewportPosIndex));
        }
        if (GUILayout.Button("Jump To End", GUILayout.Height(30)))
        {
            Adapter.JumpToEnd(ParseDurationOrDefault(_jumpDurationStr, JumpDurationDefault), JumpEase, ViewportPosToJumpPosition(_viewportPosIndex));
        }
        GUILayout.EndHorizontal();
