using UnityEngine;
using OSK.Bindings;
using UnityEditor; // Ví dụ nếu muốn bind tới UI
...
public class DemoObservableFeatures : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        _disposer.Add(_scoreProgressText.BindTo(progress =>
        {
            Debug.Log($"[COMBINE] Progress Text Updated: {progress}");
        }));
        ...
        _disposer.Add(PlayerScore.Where(score => score == 100, score =>
        {
            Debug.Log($"[WHERE] CONGRATS! Score hit the exact milestone: {score}");
        }));
        ...
        _disposer.Add(SearchInput.DistinctUntilChanged(input =>
        {
            Debug.Log($"[DISTINCT] Perform Search Query: '{input}'");
        }));
        ...
        _disposer.Add(PlayerScore.Throttle(0.5f, score =>
        {
            Debug.Log($"[THROTTLE] High-cost action (e.g., VFX update) triggered for score: {score}");
        }));
    }
